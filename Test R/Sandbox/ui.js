    ...
    if (checkApp.LicensedExpired || checkApp.planLimitReached || checkApp.wizardSetupNotCompleted || checkApp.AppUninstalled || checkApp.AppDisabled) {
        ...
        let prefferedCheckoutButtons =
            `<div class="buttonsWrapper">
                ...
                ${ sddEnabled == 'true' ? `<button id="click-and-collect-checkout-samedaydelivery-button" class="button button--primary sameday">Same Day Delivery</button>`:`` }
            </div>`;
        ...
        let sameDayDeliveryProductHTML = 
            `
            <div class="storeSection sdd-section">
        ...
        function sameDayDeliveryOrders() {
            if($('#same-day-delivery-page').length>0){
                ...
                let request = {
                    BCContext: "stores/qbuwkvkdhk", //change this when going live
                        ...
                        getProductsByProductIds(productId).then(resProdcuts => {
                            ...
                            data.forEach(function (dataElements) {
                                ...
                                if(eligibleForSDD == 1){
                                    ...
                                    $(`<span class="sdd-cart-availability unavailable-sdd">Unavailable for Same Day Delivery</span>`).insertAfter($(`[productid="${dataElements.entityId}"]`).next('span').next('.availability'));
                                }
                            });
        ...
        if (window.location.pathname == '/checkout') {
                                    ...
                                    if ($($node[0]).hasClass('shippingOptions-container form-fieldset') ||
                                        ...
                                        $('.shippingOption-desc').each(function () {
                                            ...
                                            if ($('.buttonsWrapper button[disabled]').html() == 'Pickup in Store') {
                                                ...
                                                setTimeout(() => {
                                                    // $('[name*="SAME DAY"]').trigger('click');
                                                });
                                            }
                                            else {
                                                if ($('.shippingOptions-container ul li label:not([name*="Pickup"],[name*="SAME DAY"]):first').length > 0) {
                                                    // alert('pickup mothod exists');
                                                    if ($('.shippingOptions-container ul li.optimizedCheckout-form-checklist-item--selected label[name*="Pickup"],[name*="SAME DAY"]').length > 0) {
                                                        $('.shippingOptions-container ul li label:not([name*="Pickup"],[name*="SAME DAY"]):first').trigger('click');
                                                    }
                                                    $('#checkout-shipping-continue').removeAttr('disabled');
                                                }
                                                else {
            ...
            function continueWithShipping() {
                document.getElementById("checkout-shipping-continue").addEventListener('click', function () {
                    setTimeout(function () {
